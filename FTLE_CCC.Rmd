...
gpsFiles <- c(here('ACCESS_West_Stations.csv'))
gps <- bind_rows(lapply(gpsFiles, read_csv,show_col_types = FALSE))
gps$UTC<-as.POSIXct(gps$UTC,format='%m/%d/%Y %H:%M')
```
...
currentNc \<- gpsToHfradarDownload(gps, days=intDays,
buffer=coordBuffer, name='ADRIFT067')

```{r, eval=FALSE}
# HFRADAR
currentNc <- gpsToHfradarDownload(gps, days=intDays, buffer=coordBuffer, name='ACCESS_0923')
```
...
**We can use the token "noaadgov" for direct data uploads.**
...
# restoredNc <- 'CCC_0723_HFRADAR_US_West_Coast_6km_Resolution_Hourly_Restored.nc'
restoredNc <- 'ACCESS_0923_HFRADAR_US_West_Coast_6km_Resolution_Hourly_Restored.nc'
...
(<http://transport.me.berkeley.edu/trace/>). Hit "Scan Data" and ensure
it gives a positive response.

Settings for this tool are slightly more complicated, I created a helper
function to report the proper number of values for some of the settings,
`traceSettings`. Its inputs are the restored NetCDF file, `days`, the
integration days we assigned earlier, and `increment`, which is how far
apart (hours) we want each FTLE frame to be. Lower values of `increment`
will take more total calculation time, 4 might be a reasonable starting
place.
...
-   Initial Time - auto-fills from "Scan Data" - see note in "Time
    Direction" below
...
-   Time direction - forward for **repelling** FTLE, check backward for
    **attracting** FTLE. **NOTE** if you check backward, add time to
    **Initial time** equal to **Integration duration**
...
-   Output Variables - Check output trajectory coordinates at
    observation times
-   Output Variables - Check Compute FTLE on trajectory
-   Array structure - Select Grid-indexed arrays
-   Visualization - Select none, I ran into errors trying to visualize
    in the web app

Hit **Run**! This will take a while, but I've noticed that the web app
can lose connection (and thus not finish calculations) if you leave your
computer for too long. I've made a habit of checking on the calculations
every couple minutes to make sure its still running.

Once finished hit download and move the file back to this project
directory. I recommend renaming it to have "forward" or "backward" in
the name depending on the time direction selected.

### Working With FTLE Output

Now we can read the FTLE data in to R with helper function `traceToDf`.
This needs to know if it was run forward or backward, and the number of
integration days (these are used to properly assign times - times for
each FTLE frame are set to be the middle of the integration period).

```{r, cache=TRUE}
ftleFile <- here('ACCESS_0923_Results.nc')
# this can take a while and creates a very large dataframe
ftle <- traceToDf(ftleFile, backward=FALSE, days=intDays)
```

This dataframe contains data for *all* time frames, to make a single
plot we want to look at one at a time, so we can filter by the `time`
column. Here lets plot the first time frame.

```{r}
plotFTLE(ftle[ftle$time == unique(ftle$time)[1], ], gps=gps, gpsGroup='Line',showPath=FALSE)
```

Add wind energy area

```{r}
Map<-plotFTLE(ftle[ftle$time == unique(ftle$time)[1], ], gps=gps)

WEA<-readRDS(here('WindCallBoundary.rdata'))

Map+geom_sf(data=WEA, fill=NA, col='purple',lwd=1)+geom_point(data=gps,aes(Longitude,Latitude),color='orange')
```

Add station labels

```{r}
Map<-plotFTLE(ftle[ftle$time == unique(ftle$time)[1], ], gps=gps,showPath=FALSE, gpsGroup='Line')
Map+geom_point(data=gps,aes(Longitude,Latitude))+geom_text(data=gps,aes(Longitude,Latitude,label=Line),size=3,hjust=2.5)
```

By default this uses the full extent of lat/long ranges in the GPS and
FTLE data, we can restrict the plot range with `xlim` and `ylim`
parameters.

```{r}
Map<-plotFTLE(ftle[ftle$time == unique(ftle$time)[8], ], gps=gps,
         xlim = c(-125, -122), ylim=c(37, 39),showPath=FALSE)
Map<-Map+geom_point(data=gps,aes(Longitude,Latitude))+geom_text(data=gps,aes(Longitude,Latitude,label=Line),size=2,hjust=-.5)
```

You can also create a GIF of the full time duration with the
`plotFTLEGIF` function, this takes a bit of time to run.

```{r, eval=FALSE}
plotFTLEGIF(ftle, gps=gps, title='ACCESS_0923',gpsGroup='Line',showPath=FALSE,progress=TRUE,file='ACCESS_0923_forward.gif')
```

You can also create side-by-side plots of the forward & backward FTLEs
if you load in both files and combine them into a single dataframe.
